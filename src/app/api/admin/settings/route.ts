...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const [{ data: pointData, error: pointError }, { data: orgData, error: orgError }] = await Promise.all([
      db.from("inspection_points")
        .update(point)
        .eq("id", pointId)
        .select("id,name,address,medic_surname,mechanic_surname,medical_exam_price,mechanic_exam_price")
        .maybeSingle(),
      db.from("system_settings").upsert(organization).select("id,organization_name,organization_address,organization_bank_account,organization_unp,organization_phone,organization_email,director_name").maybeSingle(),
    ]);

    if (pointError) throw pointError;
    if (orgError) throw orgError;
    if (!pointData) return NextResponse.json({ error: "Пункт осмотра не найден" }, { status: 404 });

    return NextResponse.json({ ok: true, point: pointData, organization: orgData });
  } catch (error) {
