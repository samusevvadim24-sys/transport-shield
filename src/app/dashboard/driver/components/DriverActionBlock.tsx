...
export default function DriverActionBlock({latestCheck,inspectionAddress,getDisplayStatus,getStatusStyle,hasExpiredDocs,timeRemaining,isSubmitting,onRequest}:Props){
  ...
  return <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
    <div className="flex items-start justify-between gap-3"><h3 className="text-xs font-semibold uppercase tracking-wider text-slate-500">Действия</h3>{latestCheck&&<span className={`max-w-[75%] rounded-full border px-2.5 py-1 text-right text-xs font-semibold leading-4 ${getStatusStyle(status)}`}>{getDisplayStatus(status,inspectionAddress)}</span>}</div>
    {waiting?<div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-xs font-medium text-amber-700"><Clock size={18} className="animate-pulse shrink-0"/><span>Ваш запрос обрабатывается. Пожалуйста, ожидайте.</span></div>
      ...
      :<button type="button" disabled={isSubmitting} onClick={onRequest} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#042433] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition hover:bg-[#031d29] disabled:opacity-70">{isSubmitting?<span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white"/>:<CheckCircle2 size={16}/>} {isSubmitting?"Отправка...":"Пройти осмотр"}</button>}
  </div>;
}
