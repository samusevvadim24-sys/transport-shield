...
export default function DriversTab() {
  ...
  const handleDriverHistory = (driver: Driver) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (error) {
        setDeleteError(error.message || "Не удалось удалить водителя. Попробуйте ещё раз.");
        return;
      }
      ...
      setDrivers((prev) => prev.filter((driver) => driver.id !== deletingDriver.id));
      setTotalRecords((prev) => Math.max(0, prev - 1));
    } catch (error: any) {
        ...
        <div className="w-full space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Управление штатом водителей, документами и автомобилями</p>
            </div>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <input accept=".xlsx,.xls" className="hidden" type="file" />
              <button type="button" className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 shadow-sm transition-colors hover:bg-slate-50">
                <Upload size={18} /> Импорт Excel
              </button>
              <button type="button" onClick={handleAddDriver} className="flex cursor-pointer items-center justify-center gap-2 rounded-lg bg-[#042433] px-4 py-2.5 text-sm font-medium text-white shadow-sm transition-colors hover:bg-[#031d29]">
                <Plus size={18} /> Добавить водителя
              </button>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
            <div className="relative w-full sm:w-80">
              ...
              <input type="text" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Поиск по ФИО, номеру, авто..." className="w-full rounded-lg border border-slate-300 bg-slate-50 py-2 pl-9 pr-8 text-sm outline-none transition-all focus:border-[#042433] focus:bg-white focus:ring-1 focus:ring-[#042433]" />
              {search && <button type="button" onClick={() => setSearch("")} className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md p-1 text-slate-400 hover:bg-slate-200 hover:text-slate-700">×</button>}
            </div>
          </div>

          <div className="hidden overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm md:block">
            {loading ? (
              <div className="flex min-h-60 items-center justify-center"><div className="text-sm text-slate-400">Загрузка водителей...</div></div>
            ) : drivers.length === 0 ? (
              <div className="flex min-h-60 flex-col items-center justify-center px-5 text-center"><div className="text-sm font-semibold text-slate-700">{debouncedSearch ? "По вашему запросу ничего не найдено" : "Водителей пока нет"}</div></div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[1000px] text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {drivers.map((driver) => (
                      <tr key={driver.id} className="transition-colors hover:bg-slate-50/80">
                        <td className="px-4 py-4">
                          <button type="button" onClick={() => handleDriverHistory(driver)} className="group flex min-w-0 cursor-pointer items-center gap-2 text-left" title="История осмотров">
                            <span className="inline-flex items-center justify-center rounded-md bg-slate-100 px-2 py-1 font-mono text-xs font-semibold text-slate-700 shadow-sm transition-colors group-hover:bg-slate-200">№{driver.driver_id || driver.id}</span>
                            <span className="truncate font-semibold text-slate-900 transition-colors group-hover:text-[#042433]" title={driver.name || ""}>{driver.name || "Без имени"}</span>
                            <History size={14} className="shrink-0 text-slate-300 opacity-0 transition-opacity group-hover:opacity-100" />
                          </button>
                        </td>
                        ...
                        <td className="px-4 py-4 text-xs text-slate-700">
                          <div className="flex min-w-0 items-center gap-1.5"><Car size={14} className="shrink-0 text-slate-400" /><span className="truncate font-medium text-slate-800" title={driver.car_brand || ""}>{driver.car_brand || "Марка не указана"}</span></div>
                          <div className="mt-0.5 font-mono font-semibold text-slate-500"><span className="inline-block rounded border border-slate-200 bg-slate-100/50 px-1.5 py-0.5">{driver.car_number || "Гос. номер не указан"}</span></div>
                        </td>
          ...
          {totalPages > 1 && <div className="mt-4 flex flex-col items-center justify-between gap-3 border-t border-slate-200 px-2 pt-4 sm:flex-row">
            ...
            <div className="flex max-w-full items-center gap-1 overflow-x-auto px-1">{pages.map((page, index) => page === "..." ? <span key={`separator-${index}`} className="px-1 text-xs text-slate-400">...</span> : <button key={page} type="button" onClick={() => setCurrentPage(page as number)} className={`h-8 w-8 shrink-0 cursor-pointer rounded-xl text-xs font-medium transition-colors sm:h-9 sm:w-9 sm:text-sm ${currentPage === page ? "bg-[#042433] text-white shadow-sm" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}>{page}</button>)}</div>
