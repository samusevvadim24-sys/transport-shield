...
export default function DriverModal({ isOpen, onClose, driver, customers, onSave }: DriverModalProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    if (driver) {
      const existingNumber = driver.driver_id || driver.user?.login || "";
  ...
  const selectCustomer = async (customer: CustomerOption) => {
    ...
    if (!driver) {
      ...
      if (result.error || !result.number) { setFormData((prev) => ({ ...prev, customer_id: String(customer.id), driver_id: "", login: "" })); setError(result.error?.message || "Не удалось определить номер водителя"); return; }
      setFormData((prev) => ({ ...prev, customer_id: String(customer.id), driver_id: result.number, login: result.number }));
    }
  };
