...
export default function RejectInspectionModal({ isOpen, inspection, rawAlcoholDigits, rejectReasons, onClose, onAlcoholKeyDown, getFormattedAlcoholNumber, setRejectReasons, onExecute }: RejectInspectionModalProps) {
  ...
  const handleExecute = async () => {
    ...
    // Только механик: медицинские параметры не нужны и не сохраняются.
    onExecute();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-xs">
      <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="my-4 space-y-4">
          ...
          {showMechanic && (
            <div className="rounded-xl border border-slate-200 bg-slate-50/50 p-3.5"><div className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-wide text-amber-800"><Wrench size={15} /><span>Технический осмотр (Замечания)</span></div><div className="grid grid-cols-1 gap-2 text-xs text-slate-700"><label className="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white p-2 border border-slate-200 hover:bg-slate-50 transition-colors"><input type="checkbox" checked={rejectReasons.firstAidKit} onChange={(e) => setRejectReasons({ ...rejectReasons, firstAidKit: e.target.checked })} className="rounded border-slate-300 text-[#042433] focus:ring-[#042433]" /><span>Отсутствие аптечки</span></label><label className="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white p-2 border border-slate-200 hover:bg-slate-50 transition-colors"><input type="checkbox" checked={rejectReasons.extinguisher} onChange={(e) => setRejectReasons({ ...rejectReasons, extinguisher: e.target.checked })} className="rounded border-slate-300 text-[#042433] focus:ring-[#042433]" /><span>Отсутствие огнетушителя</span></label><label className="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white p-2 border border-slate-200 hover:bg-slate-50 transition-colors"><input type="checkbox" checked={rejectReasons.baldTires} onChange={(e) => setRejectReasons({ ...rejectReasons, baldTires: e.target.checked })} className="rounded bg-white p-2 border border-slate-200 hover:bg-slate-50 transition-colors" /><span>Лысая резина</span></label><label className="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white p-2 border border-slate-200 hover:bg-slate-50 transition-colors"><input type="checkbox" checked={rejectReasons.bodyDamage} onChange={(e) => setRejectReasons({ ...rejectReasons, bodyDamage: e.target.checked })} className="rounded border-slate-300 text-[#042433] focus:ring-[#042433]" /><span>Повреждение кузова или салона</span></label><label className="flex cursor-pointer items-center gap-2.5 rounded-lg bg-white p-2 border border-slate-200 hover:bg-slate-50 transition-colors"><input type="checkbox" checked={rejectReasons.lightsFault} onChange={(e) => setRejectReasons({ ...rejectReasons, lightsFault: e.target.checked })} className="rounded border-slate-300 text-[#042433] focus:ring-[#042433]" /><span>Неисправность световых приборов</span></label></div><p className="mt-2 text-[11px]">{mechanicNotAdmitted ? <span className="font-medium text-rose-600">Статус механика: Не допущен</span> : <span className="font-medium text-emerald-600">Статус механика: Допущен</span>}</p></div>
          )}
        </div>
