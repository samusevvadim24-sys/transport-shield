  ...
  return (
    <tr className={`group transition-all hover:bg-slate-50/80 ${isApproved ? "bg-emerald-50/20" : ""}`}>
      <td className="px-4 py-4 text-center align-middle">
        <span className="inline-flex items-center justify-center rounded-md bg-slate-100 px-2 py-1 font-mono text-xs font-semibold text-slate-700 shadow-sm">
          №{item.id}
        </span>
      </td>
      ...
      <td className="px-4 py-4 align-middle">
        ...
        <button
          type="button"
          className="mt-1 inline-flex max-w-full cursor-pointer items-center gap-1.5 text-left text-xs font-medium text-slate-500 transition-colors hover:text-[#042433] focus:outline-none focus-visible:rounded focus-visible:ring-2 focus-visible:ring-[#042433]/20 disabled:cursor-default disabled:hover:text-slate-500"
          onClick={() => onCustomerClick?.(item.customer)}
          disabled={!onCustomerClick}
          title={onCustomerClick ? "Показать водителей этого заказчика" : undefined}
        >
      ...
      <td className="px-4 py-4 align-middle">
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-2 text-xs text-slate-700">
            ...
            <span>
              ВУ: <span className="font-semibold text-slate-900">{item.documents.license}</span>
            </span>
            <span className="text-slate-300">|</span>
            <span className="text-slate-500">до {item.documents.licenseExpires}</span>
          </div>

          <div className="flex flex-wrap items-center gap-2 text-xs">
            <div className="flex items-center gap-1.5 rounded-md border border-slate-200 bg-white px-2 py-0.5 shadow-sm">
              <Car size={13} className="text-slate-400" />
              <span className="font-mono font-medium tracking-wide text-slate-800">
                {item.car.number}
              </span>
            ...
            <div className="flex items-center gap-2 text-[11px] text-slate-500">
              <span className="rounded bg-slate-100 px-1.5 py-0.5">Мед: {item.documents.medical}</span>
              <span className="rounded bg-slate-100 px-1.5 py-0.5">ТО: {item.documents.inspection}</span>
            </div>
          </div>
        </div>
      </td>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex flex-col justify-between rounded-lg border border-slate-100 bg-white p-2.5 shadow-sm">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-xs font-medium text-slate-800">{item.medic}</div>
          ...
          <div className="flex flex-col justify-between rounded-lg border border-slate-100 bg-white p-2.5 shadow-sm">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-xs font-medium text-slate-800">{item.mechanic}</div>
      ...
      <td className="px-4 py-4 text-center align-middle">
        <div className="flex flex-col items-center gap-2">
          <span className={`inline-flex items-center rounded-full border px-3 py-1 text-xs font-semibold shadow-sm ${getStatusBadge(item.status)}`}>
            {item.status}
          </span>
