...
export default function DriverHistoryModal({ isOpen, driver, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetchDriverHistory(driver.id, startDate, endDate).then((result) => {
      if (cancelled) return;
      ...
      setItems([]);
      setTotalCount(0);
      setCounts({ approved: 0, rejected: 0, waiting: 0 });
      setLoading(false);
    });
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-slate-500">
                  <span>Водитель: {driver.name || "—"}</span> · <span>Заказчик: {driver.customer?.name || "—"}</span>{driver.car_brand || driver.car_number ? ` · ${driver.car_brand || ""}${driver.car_number ? ` ${driver.car_number}` : ""}` : ""}
                </p>
              </div>
            </div>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <input value={startDate} onChange={(e) => setStartDate(e.target.value)} type="date" className="w-[128px] rounded border border-slate-300 bg-white px-2 py-1 text-xs outline-none focus:border-[#042433] focus:ring-1 focus:ring-[#042433]" />
                <span className="text-slate-400">—</span>
                <input value={endDate} onChange={(e) => setEndDate(e.target.value)} type="date" className="w-[128px] rounded border border-slate-300 bg-white px-2 py-1 text-xs outline-none focus:border-[#042433] focus:ring-1 focus:ring-[#042433]" />
              </div>
              <button type="button" onClick={() => { const d = new Date(); setStartDate(toDateInput(new Date(d.getFullYear(), d.getMonth(), 1))); setEndDate(toDateInput(d)); }} className="cursor-pointer rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-medium text-slate-600 shadow-sm hover:bg-slate-50 hover:text-[#042433]">Этот месяц</button>
              <span className="hidden text-[10px] text-slate-400 lg:inline">{periodLabel}</span>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-medium text-slate-600">Всего: <strong>{totalCount}</strong></span>
              <span className="rounded-full border border-[#2F855A]/20 bg-[#2F855A]/10 px-2.5 py-1 text-xs font-medium text-[#2F855A]">Допущен: <strong>{counts.approved}</strong></span>
              <span className="rounded-full border border-[#C53030]/20 bg-[#C53030]/10 px-2.5 py-1 text-xs font-medium text-[#C53030]">Не допущен: <strong>{counts.rejected}</strong></span>
              <span className="rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-700">Ожидание: <strong>{counts.waiting}</strong></span>
            </div>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-200">
                  {items.map((item) => {
                    const medStatus = statusText(item.medical_status);
                    const mechStatus = statusText(item.mechanic_status);
                    const overall = statusText(item.overall_status);
                    return (
                      <tr key={item.id} className="transition-colors hover:bg-slate-50/60">
                        <td className="px-3 py-3"><div className="font-medium text-slate-800">{formatDateTime(item.completed_at || item.requested_at)}</div><div className="font-mono text-[10px] text-slate-400">{item.completed_at ? new Date(item.completed_at).toISOString().slice(0, 10) : "—"}</div></td>
                        <td className="px-3 py-3"><div className="flex items-center gap-1.5"><Stethoscope size={12} className="text-[#042433]/70" /><div><div className="text-xs font-medium text-slate-700">Администратор</div><div className="text-[10px] text-slate-400">{item.medical_date ? new Date(item.medical_date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" }) : "—"}</div></div></div><div className={`mt-1 text-[10px] font-semibold ${medStatus === "Допущен" ? "text-[#2F855A]" : medStatus === "Не допущен" ? "text-[#C53030]" : "text-amber-700"}`}>{medStatus}</div></td>
                        <td className="px-3 py-3">{item.breathalyzer_value == null ? <span className="text-xs text-slate-400">—</span> : <span className={`text-xs font-medium ${item.breathalyzer_value > 0 ? "text-[#C53030]" : "text-[#2F855A]"}`}>{Number(item.breathalyzer_value).toFixed(2)} мг/л</span>}</td>
                        <td className="px-3 py-3"><div className="flex items-center gap-1.5"><Wrench size={12} className="text-amber-500" /><div><div className="text-xs font-medium text-slate-700">Администратор</div><div className="text-[10px] text-slate-400">{item.mechanic_date ? new Date(item.mechanic_date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" }) : "—"}</div></div></div><div className={`mt-1 text-[10px] font-semibold ${mechStatus === "Допущен" ? "text-[#2F855A]" : mechStatus === "Не допущен" ? "text-[#C53030]" : "text-amber-700"}`}>{mechStatus}</div></td>
                        <td className="px-3 py-3 text-center"><span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${statusClass(overall)}`}>{overall}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
