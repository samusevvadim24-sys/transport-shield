    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/50 p-3 backdrop-blur-sm" onClick={onClose}>
      <div className="flex max-h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex items-center justify-between">
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-slate-500">Водитель: {driver.name || "—"} · Заказчик: {driver.customer?.name || "—"}{driver.car_brand || driver.car_number ? ` · ${driver.car_brand || ""}${driver.car_number ? ` ${driver.car_number}` : ""}` : ""}</p>
              </div>
            </div>
            <button type="button" onClick={onClose} className="cursor-pointer rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600" aria-label="Закрыть"><X size={18} /></button>
          </div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          {loading ? <div className="flex min-h-40 items-center justify-center text-sm text-slate-400">Загрузка истории...</div> : error ? <div className="rounded-lg border border-rose-100 bg-rose-50 p-3 text-sm text-rose-700">{error}</div> : visibleItems.length === 0 ? <div className="flex min-h-40 items-center justify-center rounded-lg border border-dashed border-slate-200 text-sm text-slate-400">За выбранный период прохождений нет</div> : (
              ...
              <table className="w-full min-w-[700px] text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-200">
                  {visibleItems.map((item) => {
                    const overall = statusText(item.overall_status); const medicalStatus = statusText(item.medical_status); const mechanicStatus = statusText(item.mechanic_status); const isAppear = overall === "Явиться";
                    ...
                    return <tr key={item.id} className="transition-colors hover:bg-slate-50/60">
                      ...
                      <td className="px-3 py-3">{isAppear ? <span className="text-xs text-slate-400">—</span> : <><div className="flex items-center gap-1.5"><Stethoscope size={12} className="text-[#042433]/70" /><div><div className="text-xs font-medium text-slate-700">Администратор</div><div className="text-[10px] text-slate-400">{item.medical_date ? new Date(item.medical_date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" }) : "—"}</div></div></div><div className={`mt-1 text-[10px] font-semibold ${statusTextClass(item.medical_status)}`}>{medicalStatus}</div>{pressure && <div className="mt-0.5 text-[10px] font-mono text-slate-500">АД {pressure}</div>}</>}</td>
                      <td className="px-3 py-3">{isAppear ? <span className="text-xs text-slate-400">—</span> : item.breathalyzer_value === null ? <span className="text-xs text-slate-400">—</span> : <span className={`text-xs font-medium ${item.breathalyzer_value > 0 ? "text-[#C53030]" : "text-[#2F855A]"}`}>{item.breathalyzer_value.toFixed(2)} мг/л</span>}</td>
                      <td className="px-3 py-3">{isAppear ? <span className="text-xs text-slate-400">—</span> : <><div className="flex items-center gap-1.5"><Wrench size={12} className="text-amber-500" /><div><div className="text-xs font-medium text-slate-700">Администратор</div><div className="text-[10px] text-slate-400">{item.mechanic_date ? new Date(item.mechanic_date).toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" }) : "—"}</div></div></div><div className={`mt-1 text-[10px] font-semibold ${statusTextClass(item.mechanic_status)}`}>{mechanicStatus}</div></>}</td>
                      <td className="px-3 py-3 text-center"><span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-semibold ${statusClass(item.overall_status)}`}>{overall}</span></td>
                    </tr>;
                  })}
                </tbody>
              </table>
        ...
        <div className="flex items-center justify-end border-t border-slate-100 bg-white px-5 py-2"><button type="button" onClick={onClose} className="rounded-md px-3 py-1.5 text-xs font-medium text-slate-500 hover:bg-slate-100 hover:text-slate-700">Закрыть</button></div>
      </div>
    </div>
  );
}
