...
const NavButton = ({
  ...
  <button
    onClick={onClick}
    className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-300 ease-in-out ${
      isActive
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-slate-50 text-slate-900 overflow-hidden">
      ...
      <aside
        ...
        } flex-col border-r border-slate-200 bg-white transition-all duration-300 ease-in-out overflow-hidden shrink-0`}
      >
        ...
        <div className="border-t border-slate-100 p-2 shrink-0">
          <button onClick={handleLogout} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-red-50 hover:text-red-600 transition-all">
            <div className="flex w-5 justify-center"><LogOut size={18} /></div>
            <span className={`whitespace-nowrap transition-all duration-300 ${isSidebarOpen ? "w-auto opacity-100 delay-150" : "w-0 opacity-0 overflow-hidden"}`}>Выйти</span>
          </button>
        </div>
      </aside>
      ...
      <nav className="flex md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-slate-200 px-2 justify-around items-center z-50 shadow-lg">
        ...
        <button onClick={() => { setSelectedCustomerId(""); setActiveTab("drivers"); }} className={`flex flex-col items-center flex-1 py-1 text-xs font-medium ${activeTab === "drivers" ? "text-[#042433]" : "text-slate-400"}`}>
          <Users size={20} className="mb-1" /> Водители
        </button>
        <button onClick={() => setActiveTab("customers")} className={`flex flex-col items-center flex-1 py-1 text-xs font-medium ${activeTab === "customers" ? "text-[#042433]" : "text-slate-400"}`}>
          <Building2 size={20} className="mb-1" /> Заказчики
        </button>
      </nav>
    </div>
  );
