...
import { Activity, AlertTriangle, Building2, Car, CheckCircle2, ChevronLeft, ChevronRight, Clock3, FileCheck2, LogOut, Menu, Search, Users, Wallet, X, XCircle } from "lucide-react";
...
const normalizeStatus = (value: any): string => { const status = normalizeString(value).toLowerCase().replace(/\s+/g, ""); if (status === "допущен") return "Допущен"; if (["недопущен", "отстранен", "отстранён"].includes(status)) return "Не допущен"; if (status === "ожидание") return "Ожидание"; return normalizeString(value); };
...
export default function CustomerPage() {
  ...
  const [selectedDriver, setSelectedDriver] = useState<CustomerDriver | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        let customer: { id: number; name: string; type: string | null; balance: number } | null = null;
        if (user.customer_id) {
          const { data, error } = await supabase.from("customers").select("id, name, type, balance").eq("id", user.customer_id).maybeSingle();
          if (!error && data) customer = data as typeof customer;
        }
        if (!customer && user.id) {
          const { data, error } = await supabase.from("customers").select("id, name, type, balance").eq("user_id", user.id).maybeSingle();
          if (!error && data) customer = data as typeof customer;
        }
        if (customer) {
          const companyName = [normalizeString(customer.type), normalizeString(customer.name)].filter(Boolean).join(" ");
          setCustomerProfile({ name: companyName });
      ...
      } catch (error) { console.error("Ошибка загрузки профиля заказчика:", error); setDrivers([]); }
  ...
  useEffect(() => {
    ...
    const loadInspections = async () => {
      ...
      try {
        ...
        setAllChecks(data.map((item) => { const date = item.requested_at ? new Date(item.requested_at) : new Date(); return { ...item, overall_status: normalizeStatus(item.overall_status), medical_status: normalizeStatus(item.medical_status), mechanic_status: normalizeStatus(item.mechanic_status), dateISO: getFormattedISO(date), time: date.toTimeString().slice(0,5) } as CheckData; }));
      } catch (error) { console.error("Ошибка загрузки осмотров:", error); setAllChecks([]); } finally { setChecksLoading(false); }
    };
    loadInspections();
  ...
  const selectedDriverChecks = selectedDriver ? monthChecks.filter(c => c.driver_id === selectedDriver.id) : [];
  const selectedDriverStats = {
    approved: selectedDriverChecks.filter(c => c.overall_status === "Допущен").length,
    rejected: selectedDriverChecks.filter(c => c.overall_status === "Не допущен").length,
    pending: selectedDriverChecks.filter(c => c.overall_status === "Ожидание").length,
  };
  ...
  return (<div className="min-h-screen bg-[#f5f7fa] text-slate-900">
    ...
    {selectedDriver && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" onClick={()=>setSelectedDriver(null)}><div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-3xl bg-white shadow-2xl" onClick={e=>e.stopPropagation()}><div className="sticky top-0 z-10 flex items-start justify-between border-b border-slate-100 bg-white p-6"><div><div className="text-xs font-bold uppercase tracking-wider text-slate-400">Сводка за месяц</div><h2 className="mt-1 text-2xl font-bold">{selectedDriver.name || "Без имени"}</h2><div className="mt-1 text-sm text-slate-500">{selectedDriver.car_brand || "Автомобиль"}{selectedDriver.car_number ? ` • ${selectedDriver.car_number}` : ""} · {monthLabel}</div></div><button type="button" onClick={()=>setSelectedDriver(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700"><X size={20}/></button></div><div className="p-6"><div className="grid grid-cols-3 gap-3"><div className="rounded-2xl bg-emerald-50 p-4"><div className="text-2xl font-bold text-emerald-700">{selectedDriverStats.approved}</div><div className="mt-1 text-xs text-emerald-700/70">Допущен</div></div><div className="rounded-2xl bg-red-50 p-4"><div className="text-2xl font-bold text-red-700">{selectedDriverStats.rejected}</div><div className="mt-1 text-xs text-red-700/70">Не допущен</div></div><div className="rounded-2xl bg-amber-50 p-4"><div className="text-2xl font-bold text-amber-700">{selectedDriverStats.pending}</div><div className="mt-1 text-xs text-amber-700/70">Ожидание</div></div></div>{selectedDriverChecks.length===0?<div className="mt-6 rounded-2xl bg-slate-50 p-8 text-center text-sm text-slate-500">За выбранный месяц осмотров нет.</div>:<div className="mt-6 space-y-3">{selectedDriverChecks.map(check=><div key={check.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><div><div className="font-semibold">{new Date(check.requested_at || check.dateISO).toLocaleDateString("ru-RU",{day:"2-digit",month:"long",year:"numeric"})}</div><div className="mt-1 text-xs text-slate-400">{check.time}</div></div><div className={`rounded-full border px-3 py-1 text-xs font-bold ${statusTone(check.overall_status)}`}>{check.overall_status}</div></div><div className="mt-3 grid grid-cols-2 gap-2 border-t border-slate-100 pt-3 text-xs"><div className="flex justify-between rounded-xl bg-slate-50 p-3"><span className="text-slate-500">Медик</span><span className="font-semibold">{check.medical_status || "—"}</span></div><div className="flex justify-between rounded-xl bg-slate-50 p-3"><span className="text-slate-500">Механик</span><span className="font-semibold">{check.mechanic_status || "—"}</span></div></div></div>)}</div>}</div></div></div>}
  </div>);
}
