...
export const formatInspectionItem = (item: any): Inspection => {
  ...
  return {
    ...
    car: {
      number: item.drivers?.car_number || "—",
      brand: item.drivers?.car_brand || "—",
    },
    medic: "Медик",
...
export async function fetchInspectionsData({ currentPage, statusFilter, startDateFilter, endDateFilter, search }: FetchInspectionsParams) {
  ...
  const applySearchFilter = (query: any, searchTerm: string) => {
    if (!searchTerm) return query;
    const cleanSearch = searchTerm.trim();
    return query.not('drivers', 'is', null).or(`name.ilike.%${cleanSearch}%,car_number.ilike.%${cleanSearch}%,driver_id.ilike.%${cleanSearch}%,license_number.ilike.%${cleanSearch}%`, { foreignTable: "drivers" });
  };

  let query = supabase.from("inspections").select(baseSelect, { count: "exact" });
  if (statusFilter === "Допущен" || statusFilter === "Не допущен" || statusFilter === "Явиться") {
    ...
    if (startIso && endIso) query = query.or(`and(completed_at.gte."${startIso}",completed_at.lt."${endIso}"),and(overall_status.eq.Ожидание,requested_at.gte."${startIso}",requested_at.lt."${endIso}")`);
  ...
  if (error) {
    console.error("Ошибка при загрузке осмотров:", error.message);
    if (error.code === 'PGRST103' || error.message.includes('Requested range not satisfiable')) return { formatted: [], totalCount: count ?? 0 };
...
export async function fetchStatusCounts({ startDateFilter, endDateFilter, search }: { startDateFilter: string; endDateFilter: string; search: string }) {
  ...
  if (search && search.trim()) {
    ...
    const { data: matchedDrivers, error: driversError } = await supabase.from("drivers").select("id").or(`name.ilike.%${cleanSearch}%,car_number.ilike.%${cleanSearch}%,driver_id.ilike.%${cleanSearch}%,license_number.ilike.%${cleanSearch}%`);
    if (driversError) { console.error("Ошибка при поиске водителей для счётчиков:", driversError.message); return { Все: 0, Допущен: 0, Ожидание: 0, "Не допущен": 0, "Явиться": 0 }; }
    matchingDriverIds = (matchedDrivers || []).map((d: any) => d.id);
    if (matchingDriverIds.length === 0) return { Все: 0, Допущен: 0, Ожидание: 0, "Не допущен": 0, "Явиться": 0 };
  }
  const getCountForStatus = async (status: string | null) => {
    let query = supabase.from("inspections").select("id", { count: "exact", head: true });
    if (status === "Допущен" || status === "Не допущен" || status === "Явиться") { query = query.eq("overall_status", status); if (startIso) query = query.gte("completed_at", startIso); if (endIso) query = query.lt("completed_at", endIso); }
    else if (status === "Ожидание") { query = query.eq("overall_status", "Ожидание"); if (startIso) query = query.gte("requested_at", startIso); if (endIso) query = query.lt("requested_at", endIso); }
    ...
    if (matchingDriverIds) query = query.in("driver_id", matchingDriverIds);
    const { count, error } = await query;
    if (error) { console.error("Ошибка при подсчёте статусов:", error.message); return 0; }
    return count ?? 0;
  };
...
export async function fetchSingleInspection(recordId: string | number) {
  const { data, error } = await supabase.from("inspections").select(`*, drivers!inspections_driver_id_fkey (id, driver_id, name, license_number, license_expiry, medical_expiry, tech_inspection_expiry, car_brand, car_number, customers ( name ))`).eq("id", recordId).single();
...
export async function updateInspectionApprove(docId: string, now: string) {
  ...
  const systolic = current.blood_pressure_systolic ?? 120;
  const diastolic = current.blood_pressure_diastolic ?? 80;
