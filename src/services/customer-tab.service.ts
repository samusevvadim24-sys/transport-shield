...
export async function fetchInspectionsData({
  ...
  if (statusFilter === "Допущен" || statusFilter === "Не допущен" || statusFilter === "Явиться") {
    ...
    if (startIso && endIso) {
      ...
      query = query.or(
        `completed_at.gte."${startIso}",and(overall_status.eq.Ожидание,requested_at.gte."${startIso}")`
      );
    } else if (endIso) {
      query = query.or(
        `completed_at.lt."${endIso}",and(overall_status.eq.Ожидание,requested_at.lt."${endIso}")`
      );
    }
  }
...
export async function fetchStatusCounts({
  ...
  if (search && search.trim()) {
    ...
    const { data: matchedDrivers, error: driversError } = await supabase
      .from("drivers")
      .select("id")
      .or(
        `name.ilike.%${cleanSearch}%,car_number.ilike.%${cleanSearch}%,driver_id.ilike.%${cleanSearch}%,license_number.ilike.%${cleanSearch}%`
    ...
    if (driversError) {
      console.error("Ошибка при поиске водителей для счётчиков:", driversError.message);
      return { Все: 0, Допущен: 0, Ожидание: 0, "Не допущен": 0, "Явиться": 0 };
    }
  ...
  const getCountForStatus = async (status: string | null) => {
    ...
    if (status === "Допущен" || status === "Не допущен" || status === "Явиться") {
    ...
    if (matchingDriverIds) {
      query = query.in("driver_id", matchingDriverIds);
    }

    const { count, error } = await query;
    if (error) {
      console.error("Ошибка при подсчёте статусов:", error.message);
      return 0;
    }
    return count ?? 0;
  };
  ...
  const [all, approved, pending, rejected, summon] = await Promise.all([
    getCountForStatus(null),
    getCountForStatus("Допущен"),
    getCountForStatus("Ожидание"),
    getCountForStatus("Не допущен"),
    getCountForStatus("Явиться"),
  ]);
...
export async function fetchSingleInspection(recordId: string | number) {
  const { data, error } = await supabase
    .from("inspections")
    .select(`*, drivers!inspections_driver_id_fkey (id, driver_id, name, license_number, license_expiry, medical_expiry, tech_inspection_expiry, car_brand, car_number, customers ( name ))`)
    .eq("id", recordId)
    .single();
...
export async function deleteInspectionRecord(docId: string) {
  return await supabase.from("inspections").delete().eq("id", docId);
}

export async function updateInspectionSummon(docId: string, now: string) {
  return await supabase.from("inspections").update({
