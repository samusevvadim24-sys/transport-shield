...
export async function fetchInspectionPoints(): Promise<InspectionPoint[]> {
  ...
  return (data ?? []).map((point: any) => ({
    id: Number(point.id), name: point.name ?? "", address: point.address ?? "",
    medic_surname: point.medic_surname ?? "", mechanic_surname: point.mechanic_surname ?? "",
    medical_exam_price: Number(point.medical_exam_price ?? 0.9), mechanic_exam_price: Number(point.mechanic_exam_price ?? 0.9),
  }));
}
...
export async function fetchSystemSettings(pointId?: number | null): Promise<SystemSettings> {
  ...
  return {
    id: Number(org?.id ?? 1), inspection_point_id: Number(point.id), inspection_point_name: point.name ?? "",
    inspection_point_address: point.address ?? "", medic_surname: point.medic_surname ?? "",
    mechanic_surname: point.mechanic_surname ?? "", medical_exam_price: Number(point.medical_exam_price ?? 0.9),
    mechanic_exam_price: Number(point.mechanic_exam_price ?? 0.9), organization_name: org?.organization_name ?? "",
    organization_address: org?.organization_address ?? "", organization_bank_account: org?.organization_bank_account ?? "",
    organization_unp: org?.organization_unp ?? "", organization_phone: org?.organization_phone ?? "",
...
export async function updateInspectionPointSettings(pointId: number, values: {
  name?: string; address: string; medic_surname: string; mechanic_surname: string;
  medical_exam_price: number; mechanic_exam_price: number;
}) {
  if (!Number.isFinite(pointId) || pointId <= 0) return { data: null, error: new Error("Некорректный пункт осмотра") };
  const response = await fetch("/api/admin/settings", {
    method: "PUT", headers: { "Content-Type": "application/json" }, credentials: "include",
    body: JSON.stringify({ pointId, inspection_point_name: values.name ?? "", inspection_point_address: values.address,
      medic_surname: values.medic_surname, mechanic_surname: values.mechanic_surname,
      medical_exam_price: values.medical_exam_price, mechanic_exam_price: values.mechanic_exam_price }),
  });
...
export async function updateAdminPassword(_userId: number, currentPassword: string, newPassword: string) {
